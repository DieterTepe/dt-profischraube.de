...
;(function (root, factory) {
  ...
  function fieldOptions(name, lang) {
    ...
    for (var i = 0; i < vals.length; i++) {
      ...
      else if (name === 'matGroupM' && (F = DATA.TAU_RATIO[v])) { note = pick(F.label) + ' — τB/Rm ≈ ' + F.ratio; }
  ...
  var FIELDS = {
    ...
    r11: {
      ...
      group: 'Nachweise', type: 'bool', diagram: null, dependsOn: 'connection',
      help: {
    ...
    matGroupM: {
      ...
      help: {
        de: 'Werkstoff des Bauteils, in das die Schraube greift (bzw. der Mutter). Bestimmt das Verhaeltnis Scherfestigkeit/Zugfestigkeit tau_B/R_m und damit, welches Gewinde zuerst abschert. Nur fuer den R11-Nachweis noetig.',
        en: 'Material of the part the bolt engages (or of the nut). Sets the shear-to-tensile ratio tau_B/R_m and hence which thread strips first. Needed only for the R11 verification.',
        pt: 'Material da peça em que o parafuso engata (ou da porca). Define a relação resistência ao corte/tração tau_B/R_m e, portanto, qual rosca se arranca primeiro. Necessário apenas para a verificação R11.'
      }
    },
    Rm_M: {
