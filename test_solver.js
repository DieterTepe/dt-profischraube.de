...
/* === 24 · 2D-Schnittdarstellung (schaubild.js · SX, Masterplan 5.2) ======= */
(function () {
  var SCH = require('./schaubild.js');
  ok(typeof SCH.buildSchnitt === 'function', 'Schnitt: Export buildSchnitt');
  ok(typeof SCH.schnittBuild === 'function', 'Schnitt: Export schnittBuild');
  ok(typeof SCH.bindSchnitt === 'function' && typeof SCH.refreshSchnitt === 'function', 'Schnitt: Exporte bind/refresh');

  /* A) Integration: Presets -> Engine-Ergebnis -> Schnitt (3 Sprachen) */
  S.listPresets().forEach(function (p) {
    var R = S.computeJoint(p.input);
    if (R.status !== 'ok') return;
    ['de', 'en', 'pt'].forEach(function (Lg) {
      var h = SCH.buildSchnitt(R, p.input, { lang: Lg });
      var tag = 'Schnitt[' + p.id + ',' + Lg + ']';
      ok(typeof h === 'string' && h.length > 1500, tag + ': HTML vorhanden');
      ok(h.indexOf('<svg') >= 0, tag + ': SVG enthalten');
      ok(h.indexOf('NaN') < 0, tag + ': kein NaN');
      ok(h.indexOf('data-sx-root') >= 0, tag + ': Root-Markierung');
  ...
  ok(SCH.buildSchnitt({ status: 'invalid' }, {}, {}) === '', 'Schnitt: status!=ok -> leer');

  /* B) Parametrik des reinen Zeichners (DSV/ESV, Kegel, Fuge, Bruchlinie) */
  var DS = [3, 4, 5, 6, 8, 10, 12, 16, 20, 24, 30, 36, 39];
  var LK = [6, 12, 25, 40, 60, 90, 140, 200];
  var TP = [0, 0.25, 0.45, 0.65];
  DS.forEach(function (d) {
    var dh = Math.round(d * 1.08 * 10) / 10, dw = Math.round(d * 1.4 * 10) / 10;
    LK.forEach(function (lk) {
      ['DSV', 'ESV'].forEach(function (jt) {
        TP.forEach(function (tp) {
          [Math.round(dw * 1.15 * 10) / 10, Math.round(dw * 2.4 * 10) / 10].forEach(function (DAx) {
            [true, false].forEach(function (sp) {
              [true, false].forEach(function (br) {
                var geo = { d: d, P: 0.13 * d, l_K: lk, d_h: dh, d_w: dw, D_A: DAx, joint: jt, tanphi: tp };
                var h = SCH.schnittBuild(geo, { lang: 'de', showSplit: sp, breakLong: br });
                var tag = 'SX ' + jt + ' d' + d + ' lK' + lk + ' DA' + DAx + (sp ? ' F' : '') + (br ? ' B' : '');
                ok(typeof h === 'string' && h.indexOf('<svg') >= 0, tag + ': SVG');
                ok(h.indexOf('NaN') < 0, tag + ': kein NaN');
                ok(h.indexOf('viewBox=') >= 0, tag + ': viewBox');
                ok((h.indexOf('data-sx-chip="phi"') >= 0) === (tp > 0), tag + ': phi-Chip iff tanphi>0');
                ok(h.indexOf('sb-vals') >= 0, tag + ': Legende');
                if (jt === 'DSV') {
                  var slender = br && (lk / d > 6);
                  ok((h.indexOf('stroke-width:1.2') >= 0) === slender, tag + ': Bruchlinien iff schlank');
                }
              });
            });
          });
  ...
  /* C) Determinismus, Validierung, Dreisprachigkeit */
  var g0 = { d: 12, P: 1.75, l_K: 60, d_h: 13.5, d_w: 16.6, D_A: 40, joint: 'DSV', tanphi: 0.45 };
  ok(SCH.schnittBuild(g0, { lang: 'de' }) === SCH.schnittBuild(g0, { lang: 'de' }), 'SX: deterministisch');
  [{}, { d: 12 }, { d: 12, l_K: 60, d_h: 13.5, d_w: 16.6 }, { d: -1, l_K: 60, d_h: 13.5, d_w: 16.6, D_A: 40 }]
    .forEach(function (bad, i) { ok(SCH.schnittBuild(bad, {}) === '', 'SX: unvollstaendig #' + i + ' -> leer'); });
