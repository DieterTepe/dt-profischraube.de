...
Version : 4.9.8
Stand   : 2026-08-14
Status  : **v4.9.8 real getestet & bestätigt (Handy, 2026-08-14).** Engine validiert;
          komplette dokumentfähige Ausgabe (Druck→PDF, RTF, CSV, Schaubild-PNG +
          Wasserzeichen), Test/Voll-Gating, Registrierung/Lizenznehmer, Info-/Impressum-
          Overlay, 2D-Schnittdarstellung, Review-Härtungen und alle 15 Beispiele
          ampel-garantiert. **NEU in v4.9.8: Editions-Kennung FAIL-SAFE umgekehrt** — nur
          exakt `full` schaltet die Vollversion frei, jede andere Kennung (fehlend, leer,
          `test`, Tippfehler, beliebiger String) gilt sicher als Testversion. Alles über
          die feste `DT_EDITION`-Kennung, ein Satz Module.
          **Testbasis: 2.248.710 Assertions, 0 Fehler.**
          **► NÄCHSTER SCHRITT: Basislinie bestätigen, dann nächstes Ziel aus der offenen**
          **Roadmap (5.2) MIT DIETER priorisieren — kein Ziel ist vorentschieden.**
...
**Fertig & testverifiziert (2.248.710 Assertions, 0 Fehler):** Engine R0–R13 (zentrisch)
...
selbstprüfender Rechenweg; Verspannungsschaubild; **2D-Schnittdarstellung der Verbindung**
(SX-Modul in `schaubild.js`, v4.9.6); Ingenieur-Workstation (Speichern/Laden
...
**Review-Härtungen + Beispiel-Ampel-Garantie** (v4.9.7).
Einzeiler mit Versionen siehe 5.1; Details in Git/Changelog.

**► ALLERERSTE AUFGABE IM NÄCHSTEN CHAT:** Grün-Basislinie bestätigen
(`node test_solver.js` → **2.248.710, 0 Fehler**), dann **kein Feature eigenmächtig
beginnen**, sondern das nächste Ziel **mit Dieter aus 5.2 priorisieren** („erst besprechen,
dann coden“). Kandidaten (Reihenfolge offen): separate Hilfe-HTML (5.2·4) · Inline-SVG-Skizzen
für Geometrie-Felder (5.2·1) · Build-/Obfuskierungs-Schritt + zwei Editions-Builds (5.2·3) ·
Auto-Dimensionierung (5.3, vorgemerkt) · erklärende Schaubild-Beschriftung (5.2·5).
Erst nach Dieters Wahl: Plan kurz abstimmen, dann bauen (0.4).

**Regressions-Pflicht vor/nach jedem Baustein:** alle 15 Presets × 3 Sprachen (Solver +
Rechenweg + Schaubild + Schnitt, kein NaN) **und** volle Testsuite (2.248.710, 0 Fehler).
...
2. **Grün-Basislinie:** `node test_solver.js` → **2.248.710 (oder mehr), 0 Fehler**.
   Erst dann weiterbauen.
3. Diesen Masterplan + Wiederanknüpf-Punkt (0.2) lesen, nächstes Ziel bestätigen, dann coden.
4. **2D-Schnitt-Laborphase ist abgeschlossen (v4.9.6):** Der maßgebliche Schnitt-Code ist das
   **SX-Modul in `schaubild.js`**. Liegen `schnitt.js`/`Schnitt_Labor.html` noch im
   Projektordner, sind das **obsolete Wegwerf-Reste** der Laborphase (Stand vor den
   Legende-/Maßstab-Nachträgen) — löschen, Git hält die Historie. Sie dürfen **nie** wieder
...
│                                (fail-safe: nur exakt 'full' = voll, sonst Test — v4.9.8).
...
├── schaubild.js (DTSSchaubild) → Verspannungsschaubild + **2D-Schnittdarstellung**
│                                (SX-Modul; Live-SVG, dreisprachig; Exporte build,
│                                buildSchnitt, schnittBuild, bindSchnitt, refreshSchnitt)
...
│                                Schaubild-/Schnitt-/Bericht-Einbindung, i18n, Theme, .dt-Speichern/
...
`window.DT_EDITION`-Kennung. Module bleiben **gating-frei**. **FAIL-SAFE-Regel (seit v4.9.8,
umgekehrt gegenüber v4.9.7):** **NUR exakt `'full'` schaltet die Vollversion frei; jede andere
Kennung — fehlend, leer, `'test'`, Groß-/Kleinschreibung wie `'Full'`, Tippfehler, führendes
Leerzeichen, beliebiger String — gilt sicher als Testversion.** So kann niemand über einen
beliebigen Eintrag in der HTML die Vollversion erschleichen; nur der bewusste Build mit `'full'`
ist voll. **Umsetzung ohne Bruchstelle:** `ui.js` normalisiert die Roh-Kennung einmal zu
`EDITION = (DT_EDITION === 'full') ? 'full' : 'test'` und legt sie als `window.DT_EDITION_EFF`
ab; **alle** editionsabhängigen Prüfungen (UI-Gating, Wasserzeichen, Aktivierungsdialog) hängen
an dieser einen normalisierten Kennung, damit UI-Sperre, PNG-Wasserzeichen und Aktivierung
garantiert gemeinsam fallen. Die Helfer in `report.js` sind zusätzlich je für sich fail-safe:
`isFeatureAllowed(f,ed)` gibt nur bei `ed==='full'` frei (sonst nur PNG), `shouldWatermark(ed)`
markiert alles außer `'full'`. **Test:** Rechnen + Rechenweg + Schaubild am Schirm sichtbar,
**Ausgaben gesperrt** (nur PNG erlaubt, mit Wasserzeichen), dezenter gelber Hinweisbalken/Upsell
— auch die Testdatei wird gebündelt+obfuskiert (sonst läge der Rechenkern offen). **Voll:**
Ausgabe frei; Lizenz nach Ehrlichkeitsprinzip (siehe 4, Registrierung).
Reine Logik `isFeatureAllowed`/`shouldWatermark`/`watermarkText` in `report.js`.
Die clientseitige Kennung bleibt bewusst umgehbar (kein Kopierschutz, Ehrlichkeitsprinzip,
Review-P10) — die Fail-safe-Regel verhindert lediglich die *versehentliche* oder *triviale*
Freischaltung durch irgendeinen Eintrag.
...
- **Schaubild-Merksatz:** **Zahlenwerte gehören in die HTML-Legende unter dem SVG, nie in die
  Zeichnung.** Gilt auch für den **2D-Schnitt (umgesetzt in v4.9.6)**: Im SVG stehen nur
  Formel-Symbole; die antippbaren Legende-Chips blenden Maß-Overlays (l_K, D_A, d, d_h, d_w, φ)
  ein/aus. Feste Darstellungsregeln des Schnitts: Verformungskegel **an** (gestrichelt, aus
  Engine-tanφ, an D_A geclippt → Hülsenfall sichtbar) · Trennfuge bei l_K/2 **an** ·
  Bruchlinien **automatisch** ab l_K/d > 6 · Platzierung **unter dem Verspannungsschaubild** ·
  Bildschirm-Maßstab Start **50 %** mit dezentem Regler 25–100 % (nicht persistiert) ·
  Druck/PDF **fix 33 %**, Regler + Chip-Hinweis ausgeblendet, Block bricht nicht auseinander.
  Gleiche Geometrie-/Transformquelle wie das Schaubild, kein DXF.
...
- **`schaubild.js`:** Live-SVG des Verspannungsdreiecks, Zahlen in der Legende, rein additiv.
  Enthält seit v4.9.6 zusätzlich das **SX-Modul (2D-Schnitt)**: rein zeichnend, keine Physik,
  Styling ausschließlich über Design-Token-`var()` direkt am SVG-Element (keine
  `style.css`-Erweiterung nötig, folgt hell/dunkel automatisch); `buildSchnitt(R,inp,opts)`
  mappt Engine/Eingaben (d, P aus `R.geometry`; l_K, d_h, d_w, D_A, connection aus `inp`;
  tanφ aus `R.tanPhi` — fehlt es bei δ_P-Override, entfällt nur der Kegel), `schnittBuild`
  ist der pure Zeichner für Node-Tests, `bindSchnitt`/`refreshSchnitt` verkabeln Chips und
  Maßstab-Regler (Zustand am Container, übersteht Re-Render).
...
- **15 Beispiele** decken statisch/schwellend/wechselnd, Querkraft, Moment, kombiniert,
  ...
  **Ampel-Garantie (seit v4.9.7, Sektion 27):** kein Beispiel wird rot, jede geführte
  Sicherheit liegt ≥ 1,2, alle sind montierbar (F_Mmax ≤ F_Mzul) — testverankert.
- **Bewusst getrackte Annahmen / offene Norm-Punkte:** Kegelwinkel-Geltungsgrenzen (β_L, y) als
  ...
  (Momentfelder M_Ymax/q_M/r_a bereits vorhanden und getestet); **δ_P-Fall Kegel+Hülse**:
  Struktur nach VDI, Konsistenz getestet (3 der 15 Beispiele liegen in diesem Fall),
  externes Norm-Zahlenbeispiel mit d_w < D_A < D_A,Gr noch zu beschaffen (Dieter) und
  dann als Test zu verankern (`PENDING_DP_CONE_SLEEVE`); **F_Vmax** ist bewusste
  Info-Größe im Ergebnis (erscheint im Schaubild als F_V = F_M − F_Z, dort ohne
  ΔF_Vth-Abzug — bei Gelegenheit angleichen), kein Nachweis (R10 nutzt F_Smax).
- **Testabsicherung `test_solver.js` (DEV-ONLY): 2.248.710 Assertions, 0 Fehler.** Geometrie-
  ...
  Gating `isFeatureAllowed`, Registrierung (Sektion 25/26), **2D-Schnitt (Sektion 24: Presets ×
  3 Sprachen durch die Engine, ~6.650 Parametrik-Kombis DSV/ESV/Kegel/Fuge/Bruch, Merksatz
  „keine Zahlen im SVG“, Legende- und Maßstab-/Druck-Regressionen)**, **Review-Härtungen +
  Beispiel-Ampel (Sektion 27: lsGet/lsSet-Absturzschutz, RTF-signed-Unicode; je Preset:
  status ok · Ampel nie rot · jede geführte Sicherheit ≥ 1,2 · F_Mmax ≤ F_Mzul; **Fail-safe-
  Edition: 17 Nicht-`full`-Varianten sperren Exporte + tragen Wasserzeichen, nur exakt `full`
  schaltet frei**)**.
  **Regel: erweitern, nie lockern.**
...
✅ **v4.9.5** Info-/Impressum-Overlay · ✅ **v4.9.6** 2D-Schnittdarstellung der Verbindung
(Laborphase → SX-Modul in `schaubild.js`, Maß-Chips, Maßstab-Regler, Druck fix 33 %) ·
✅ **v4.9.7** Review-Härtungen + Beispiel-Ampel-Garantie ·
✅ **v4.9.8** Editions-Kennung fail-safe (nur exakt `full` = voll).
**Alle vom Anwender am Handy real getestet & bestätigt.**

### 5.2 Offen (nach vorn)
*Reihenfolge offen — das nächste Ziel wird **mit Dieter** priorisiert (siehe 0.2), nichts ist
vorentschieden.*
1. **Inline-SVG-Skizzen** für Geometrie-Felder (Slot via `diagram`-Attribut vorhanden):
   Fokus-Feld hebt die passende Linie/Pfeil hervor. Bewusst später und getrennt.
2. **Exzentrische Last** (σ_b; Ruoss B4/B5) — **einziger echter Engine-Eingriff der Zukunftsliste**,
   eigener gründlicher Zyklus, viel später.
3. **Build-/Obfuskierungs-Schritt** + `DT-ProfiSchraube_Pro.html`/`_Test.html` (zwei Builds,
   siehe 2.3); A7-Vordimensionierung.
4. **Separate Hilfe-HTML im GitHub-Ordner** (strategisch bevorzugt): getrennt vom Tool, offline
...
5. **Erklärende Schaubild-Beschriftung** (ruhige Legende/Kurzerklärung statt Interaktivität).

### 5.3 Vorgemerkt & Verworfen
- **Vorgemerkt (kein Beschluss):** **Klemmkraft-Transparenzzeile** — F_KR auch rein axial
  als Kennwert/Rechenweg-Zeile zeigen (Review-P2; kein Fehler: F_KR ≡ F_Kerf per
  F_Mmin-Konstruktion, daher reiner Anzeige-Mehrwert bei Dicht-/Abhebefällen).
  · **Auto-Dimensionierung** „kleinste Größe/Klasse, bei der alle
...
**v4.9.8 (2026-08-14) — Editions-Kennung FAIL-SAFE umgekehrt.**
Auslöser (Dieter): Bislang galt „nur `'test'` ist Test, **alles andere = Voll**" — ein leerer
oder beliebiger Eintrag in der HTML hätte damit die Vollversion freigeschaltet. Gewünscht und
umgesetzt ist die sichere Umkehrung: **NUR exakt `'full'` schaltet die Vollversion frei; jede
andere Kennung (fehlend, leer, `'test'`, `'Full'`, führendes Leerzeichen, Tippfehler, beliebiger
String) gilt sicher als Testversion.**
**Umsetzung ohne Bruchstelle:** In `ui.js` wird die Roh-Kennung **einmal** normalisiert
(`EDITION = (DT_EDITION === 'full') ? 'full' : 'test'`, abgelegt als `window.DT_EDITION_EFF`);
**alle** editionsabhängigen Prüfungen hängen jetzt an dieser einen Kennung. Das war nötig, weil
die Wasserzeichen-Prüfung vorher direkt an der Roh-Kennung hing — eine reine Umkehr nur an der
UI-Zeile hätte sonst bei leerer Kennung die UI gesperrt, aber das PNG **ohne** Wasserzeichen
gelassen (Widerspruch). Zusätzlich sind die Helfer in `report.js` je für sich fail-safe:
`isFeatureAllowed(f,ed)` gibt nur bei `ed==='full'` frei (sonst nur `png`), `shouldWatermark(ed)`
markiert alles außer `'full'`.
**Tests angepasst (strenger, nicht gelockert):** Die alten Zusicherungen kodierten die frühere
umgekehrte Regel („unbekannte Kennung → wie Voll / kein Wasserzeichen") — genau das erkannte
Sicherheitsleck; sie wurden auf die Fail-safe-Regel umgeschrieben. Neuer expliziter Block in
Sektion 27: 17 Nicht-`full`-Varianten sperren Speichern/Laden/Druck/RTF/CSV, erlauben nur PNG
und tragen das Wasserzeichen; Gegenprobe, dass exakt `'full'` die **einzige** freischaltende
Kennung ist. End-to-End-Simulation der UI-Regel bestätigt: `'full'` frei, `' full'`/`'FULL'`/
leer/fehlend → Test mit Sperre + Marke.
**HTML-Kommentare** in beiden Builds auf die Fail-safe-Regel korrigiert (die frühere Notiz
„alles andere = voll" war jetzt falsch). Die Kennungen selbst bleiben `'full'` bzw. `'test'`.
Suite 2.248.591 → **2.248.710**, 0 Fehler. Dateien: `ui.js`, `report.js`, `test_solver.js`,
`DT-ProfiSchraube-x-x-x.html`, `DT-ProfiSchraube_Test.html`; `style.css`/Engine-Formeln
unangetastet. **P10 bleibt bewusst clientseitig/umgehbar** (Ehrlichkeitsprinzip, kein
Kopierschutz) — die Fail-safe-Regel verhindert nur die *triviale/versehentliche* Freischaltung.
Vom Anwender am Handy real getestet & bestätigt.

**v4.9.7 (2026-07-22) — Externes Code-Review geprüft & gehärtet + Beispiel-Ampel-Garantie.**
Grundlage: fremder Mängelbericht `Fehlerbereinigun_Schraube.md` (10 Punkte). **Methodik:
jede Behauptung gegen den Original-Code verifiziert, teils empirisch gegen die laufende
Engine — nichts ungeprüft übernommen.** Ergebnis-Protokoll (damit entkräftete Punkte nie
wieder „repariert" werden):
**Umgesetzt (P3/P4/P6/P8):** `lsGet`/`lsSet`-Helfer in `ui.js` — Sprache/Theme greifen nie
mehr ungeschützt auf `localStorage` zu (Privatmodus-Absturz behoben; Helfer exportiert und
in Node getestet, dort fehlt der Speicher wie im gesperrten Browser) · Ergebnis-Banner
trägt `role="status"`/`aria-live="polite"` — Screenreader lesen das neue Urteil automatisch
vor (gezielter als der Review-Vorschlag „ganzer resultHost"; HTML-Dateien unangetastet) ·
`rtfEsc` schreibt Zeichen > 32767 normgerecht vorzeichenbehaftet (RTF `\uN` = signed
16-bit; Bestandszeichen byte-identisch, per Test fixiert) · Formular-Gruppen-Abzeichen
neutral **1–6** statt „R0–R5" — „R" gehört eindeutig den VDI-Schritten R0–R13.
**Entkräftet (P1/P9/P10):** P1 „Ampel sagt ausreichend ohne einen einzigen Nachweis" ist
**unerreichbar** — empirisch belegt: bei `status ok` wird S_F immer geführt (`hasAny` nie
false), der Vorbehalts-Text erscheint nur mit mindestens einem grün geführten Nachweis ·
P9 Regex-Verdacht: Original enthält gültiges `/^_+|_+$/g` (Kopie-Artefakt) · P10 Edition
clientseitig = dokumentierte Entscheidung (2.3). **Nachtrag v4.9.8:** P10 bleibt bewusst
clientseitig/umgehbar (Ehrlichkeitsprinzip), die Freischalt-Logik wurde aber **fail-safe
umgekehrt** — nur exakt `full` ist voll, jede andere Kennung sicher Test (siehe 2.3 und
Changelog v4.9.8).
**Bewusst belassen (P7):** stiller 10-s-Lizenz-Reset bleibt wie in v4.9.4 dokumentiert
(touchcancel bricht bei Scroll ab; nach Versehen erscheint der Aktivierungsdialog erneut).
**Backlog (P2/P5):** Klemmkraft-Transparenzzeile vorgemerkt (5.3; F_KR ≡ F_Kerf, kein
Fehler; F_Vmax erscheint im Schaubild als F_V) · Kegel+Hülse-Normbeispiel als
Beschaffungsaufgabe (Abschnitt 4, Annahmen).
**Beispiel-Überarbeitung (Dieters Auftrag: keine roten Ampeln):** 3 rote + 2 knappe
Presets fachlich korrigiert — Klemmkraftbedarf aus Querkraft/Torsion war unterdeckt:
`flansch_torsion_m16` F_Kerf 20 000 → **38 000 N** (S_G 0,71 → 1,34) ·
`getriebeflansch_m16` 20 000 → **42 000 N** (0,64 → 1,34) · `querkraft_m12` 15 000 →
**16 000 N** (1,20 → 1,28) · `kombiniert_m16` 24 000 → **26 000 N** (1,20 → 1,30) ·
`alu_flansch_dT` Scheibe **d_w 20 → 24 mm** (S_P 0,98 → 1,78). Anhalt-Validierungsanker
unangetastet. **Neue Dauergarantie (Sektion 27):** jedes Beispiel status ok, Ampel nie
rot, jede geführte Sicherheit ≥ 1,2, montierbar (F_Mmax ≤ F_Mzul).
Suite 2.248.448 → **2.248.591**, 0 Fehler (kein bestehender Test hing an den alten
Preset-Werten — nichts gelockert). Dateien: `daten.js`, `ui.js`, `report.js`,
`test_solver.js`; HTML/`style.css`/Engine-Formeln unangetastet. `Fehlerbereinigun_Schraube.md`
kann nach diesem Protokoll aus dem Projektordner entfernt werden (Git). Vom Anwender am
Handy real getestet & bestätigt.

*(Ältere Einträge v4.1.0–v4.9.6 und alle Detailprotokolle: Git-Commit-Historie bzw.
frühere Masterplan-Stände, zuletzt `Masterplan_DT-ProfiSchraube-4-9-7.md`.)*

═══════════════════════════════════════════════════════════════════════════
Ende des Masterplans · DT-ProfiSchraube v4.9.8
═══════════════════════════════════════════════════════════════════════════
