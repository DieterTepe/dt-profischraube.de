...
Version : 4.9.7
Stand   : 2026-07-22
Status  : **v4.9.7 real getestet & bestätigt (Handy, 2026-07-22).** Engine validiert;
          ...
          Overlay, **2D-Schnittdarstellung** und — neu — **externes Code-Review geprüft
          & gehärtet** (localStorage-Absturzschutz, Screenreader-Ansage, RTF-Unicode,
          eindeutige Gruppen-Nummern) sowie **alle 15 Beispiele ampel-garantiert** (nie
          rot, geführte Sicherheiten ≥ 1,2, montierbar — testverankert). Alles über die
          feste `DT_EDITION`-Kennung, ein Satz Module.
          **Testbasis: 2.248.591 Assertions, 0 Fehler.**
...
**Fertig & testverifiziert (2.248.591 Assertions, 0 Fehler):** Engine R0–R13 (zentrisch)
...
(`node test_solver.js` → **2.248.591, 0 Fehler**), dann **kein Feature eigenmächtig
...
Rechenweg + Schaubild + Schnitt, kein NaN) **und** volle Testsuite (2.248.591, 0 Fehler).
...
2. **Grün-Basislinie:** `node test_solver.js` → **2.248.591 (oder mehr), 0 Fehler**.
...
- **Testabsicherung `test_solver.js` (DEV-ONLY): 2.248.591 Assertions, 0 Fehler.** Geometrie-
  ...
  status ok · Ampel nie rot · jede geführte Sicherheit ≥ 1,2 · F_Mmax ≤ F_Mzul)**.
...
clientseitig = dokumentierte Entscheidung (2.3).
...
**v4.9.6 (2026-07-21) — 2D-Schnittdarstellung der Verbindung (Laborphase → Integration).**
**Laborphase ①** wie in v4.9.5 geplant: Übernahme-Modul `schnitt.js` (rein zeichnend, UMD,
Styling nur über Design-Token-`var()` inline am SVG) + self-contained Wegwerf-Hülle
`Schnitt_Labor.html` (Tokens beider Themes 1:1 aus `style.css`, Regler/Presets inkl.
VDI-Näherung für tanφ nur zur Labor-Vorbelegung); Optik am Handy abgenommen.
**Integration ②:** Labor-Kern **1:1 als SX-Modul** in `schaubild.js` (skriptgesteuert
injiziert); neue Exporte `buildSchnitt(R,inp,opts)` (Mapping: d/P aus `R.geometry`,
l_K/d_h/d_w/D_A/connection aus `inp`, tanφ aus `R.tanPhi`; bei δ_P-Override entfällt nur
der Kegel), `schnittBuild` (purer Zeichner für Node-Tests), `bindSchnitt`/`refreshSchnitt`
(delegierte Chip-/Regler-Verkabelung, Zustand am Container, übersteht Re-Render);
`ui.js renderViz` hängt den Schnitt mit Zwischentitel unter das Verspannungsschaubild.
**Inhalt:** maßstäblicher Längsschnitt — Sechskantkopf über Eck mit Fasenbögen, Auflage d_w,
Schaft + vereinfachtes Gewinde (dicke Außen-, dünne Kernlinie), Durchgangsloch d_h, Paket
über l_K, außen D_A; DSV mit Mutter + Gewindeüberstand, ESV mit Sackloch (Kernloch,
118°-Spitze, Restgewinde, Freihand-Unterkante); gegenläufige 45°-Schraffur, Schraube/Mutter
ungeschnitten, strichpunktierte Mittellinie; Verformungskegel gestrichelt aus Engine-tanφ,
an D_A geclippt (Hülsenfall sichtbar); Trennfuge bei l_K/2; Bruchlinien automatisch ab
l_K/d > 6. **Merksatz eingehalten:** Zahlen nur in der HTML-Legende; antippbare Chips
blenden Maß-Overlays (l_K, D_A, d, d_h, d_w, φ) ein/aus. Alle vier Labor-Entscheidungen: ja.
**Nachträge aus Handy-/Druck-Feedback:** Legende-Fix (Typ-Langtext in die Namensspalte,
φ-Name gekürzt, Umbruch-Override je Wertspalte — behebt Textüberlagerung, PT am längsten);
**Maßstab:** Bildschirm-Start **50 %** der Kartenbreite, zentriert, dezenter Regler
25–100 % im Zwischentitel (nicht persistiert, übersteht Neuberechnungen); **Druck/PDF fix
33 %**, Regler + Chip-Hinweis ausgeblendet, Block (Titel + Zeichnung + Legende)
`break-inside:avoid` — behebt ganzseitigen Druck, verwaisten Titel und abgetrennte Legende.
**Tests Sektion 24** (Presets × 3 Sprachen durch die Engine; ~6.650 Parametrik-Kombis
DSV/ESV × Kegel × Fuge × Bruch; Merksatz-, i18n-, Legende-, Maßstab-/Druck-Regressionen):
Suite 2.211.500 → **2.248.448**, 0 Fehler. Dateien: `schaubild.js`, `ui.js`,
`test_solver.js`; **HTML + `style.css` unangetastet**. Labor-Dateien (`schnitt.js`,
`Schnitt_Labor.html`) sind obsolet → aus dem Projektordner entfernen (Git hält die
Historie). Vom Anwender am Handy real getestet & bestätigt (inkl. Druckvorschau).

*(Ältere Einträge v4.1.0–v4.9.5 und alle Detailprotokolle: Git-Commit-Historie bzw.
frühere Masterplan-Stände, zuletzt `Masterplan_DT-ProfiSchraube-4-9-6.md`.)*

═══════════════════════════════════════════════════════════════════════════
Ende des Masterplans · DT-ProfiSchraube v4.9.7
═══════════════════════════════════════════════════════════════════════════
