...
(function () {
  ...
  var NOTE = {
    ...
    PENDING_R11: { en: 'Minimum engagement depth (R11): the bolt should break before the thread strips. This version only estimates roughly (~0.9·d). With a through-bolt and a standard nut of matching property class it is usually satisfied. For a tapped joint in soft material (aluminium, cast iron) ensure sufficient engagement depth (guide: steel ~1·d, cast iron ~1.4·d, aluminium ~2·d). The exact check will follow as an extension.', pt: 'Profundidade mínima de engate (R11): o parafuso deve romper antes de a rosca se arrancar. Esta versão apenas estima de forma aproximada (~0,9·d). Com parafuso passante e porca normalizada de classe adequada está, em regra, cumprido. Em união roscada em material macio (alumínio, ferro fundido) garanta profundidade de engate suficiente (referência: aço ~1·d, ferro fundido ~1,4·d, alumínio ~2·d). A verificação exata seguirá como extensão.' }
  };
  ...
  function updateRmHint() {
    var mg = fieldEls['matGroupM'], rmc = fieldEls['rmCustom'], rm = fieldEls['Rm_M'];
    var row = fieldRows['Rm_M']; if (!rm || !row) return;
    var hintEl = row.querySelector('.field-hint');
    if (rm.disabled) { rm.readOnly = false; rm.classList.remove('locked'); if (hintEl) hintEl.textContent = ''; return; }
    var grp = (mg && mg.value) ? DATA.TAU_RATIO[mg.value] : null;
    var custom = !!(rmc && rmc.checked);
    if (!custom && grp && grp.rmDefault != null) {
      rm.value = grp.rmDefault;                 // Richtwert vorbelegen
      rm.readOnly = true; rm.classList.add('locked');
      if (hintEl) hintEl.textContent = t('rmHintPrefix') + ': ' + grp.grade + ' · ' + grp.rmDefault + ' N/mm²';
    } else {
      rm.readOnly = false; rm.classList.remove('locked');
  ...
  function renderResults(R) {
    ...
    if (R.engagement) {
      ...
      function eRow(k, vHtml) { var tr = el('tr'); tr.appendChild(el('td', 'k', k)); var td = el('td', 'v'); td.innerHTML = vHtml; tr.appendChild(td); eTbl.appendChild(tr); }
